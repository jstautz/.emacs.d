...
The important part is that last s-expression. This line will export ("tangle") the code blocks from the =emacs-init.org= file into a file called =emacs-init.el=, then load that file.

That tiny =init.el= file is the only native elisp file in my whole config. Every other bit of configuration is loaded from org-mode files.
