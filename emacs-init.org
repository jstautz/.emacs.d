...
- Requires Emacs 24.5+. Only tested on Mac OS X.
...
The instructions that follow are mostly notes for my future self. Follow them at your own peril.
...
Once you've backed things up, clone the repo:

#+begin_example
$> git clone https://github.com/jeff.stautz/.emacs.d ~/.emacs.d
#+end_example
...
- Install cask (with with =curl -fsSL https://raw.githubusercontent.com/cask/cask/master/go | python= and then export PATH="/Users/jeff.stautz/.cask/bin:$PATH" ?)
- Assumes /Users/jeff.stautz/.emacs.autosave/ exists
- Install gpg
- Install aspell
- Install gzip
- Growlnotify
- Marked.app
- yas snippets assumes ~/bin/whereami (also note you need to enable location services in Privacy prefs)
- Assumes ~/org all over the place
** Fire up Emacs
:PROPERTIES:
...
Because tangling and loading my .org file takes a few seconds, I've wrapped this call to org-babel in an =if= statement that checks to see if tangling is necessary, or if I can load the .el file directly. 
...
*** TODO Package version pinning with Cask & Pallet

/As I'm working on this documentation, I noticed that Pallet and Cask now support pinning specific version numbers or git branches./ 

/This has been a pain point for me in the past (e.g. I reinstall Emacs, then spend hours fiddling with packages that broke because new versions aren't backward compatible), but I want to think about the best ways to solve this problem without rigidly defining a version for every package./
...
When use-package initializes smart-mode-line, I want to set up the theme and width right away.
...
smart-mode-line has a useful feature that allows you to replace elements of a path with an abbreviation. So instead of seeing =/Users/jeff.stautz/Documents/Work/filename.txt= in the modeline, I'll see =:Work:filename.txt=. 
...
Yes, there's some duplication in the list above. My =~/Documents/Writing= folder is symlinked to =~/Dropbox/Writing=, and smart-mode-line needs to know about both possible paths in order to work correctly.
...
Multiple-cursors solves this problem nicely. Check out this raw power:

<iframe width="560" height="315" src="https://www.youtube.com/embed/jNa3axo40qM" frameborder="0" allowfullscreen></iframe>
...
[[file:img/expand-region.gif]]

(gif credit to Aron Woost)
...
I've bound this to =C-M-p=, so it's similar to my keybinding for =other-window=, but I haven't totally bought into this keybinding just yet.
...
*** TODO note that C-<tab> doesn't currently work in org-mode because of a keybinding conflict
...
*** TODO add a gif for the above section, demonstrating how to change window sizes.
...
** Images
Emacs can display image files out of the box. However, there are limitations around certain actions. The Image+ package gives us some additional features.
...
To manipulate an image, use =M-x imagex-global-sticky-mode=, then use =C-c += and =C-c -= to zoom in and out.
** Emoji support
...
I type badly and make a lot of the same mistakes over and over again. Using abbrev-mode lets me automatically fix these. Like my phone's autocorrect, except not insane.

Let's activate it in text-mode and org-mode for now:

#+name: abbrev-mode-hooks
#+begin_src emacs-lisp
...
Let's save my abbrevs in my .emacs.d directory, and save new abbrevs to it when I save or quit. 
...
*** TODO I've built an abbrev file based on common typos from [[https://en.wikipedia.org/wiki/Wikipedia:Lists_of_common_misspellings/For_machines][here]]. I should go review those and remove some -- some of those misspellings are pretty damn dumb. 

And I also should layer in any shortcuts I want to add. Like "because" bc. (note I added BC on its own as an abbrev for itself, because I am gonna want to type BC at some point. Like, the province I live in.)
...
TO DO: Open the current region in Marked.app (stored in a temporary file).
...
wc-goal-mode lets me replicate the experience of 750words.com. I can set word-count goals for myself and then write to meet them.

One downside is that it becomes *super* slow on large files, especially large org-mode files.

Generally I'll use it on a single smaller file, or when narrowed into a sub-tree for the section of the story I'm currently working on.

#+name: writing-wc-goal
#+BEGIN_SRC emacs-lisp
...
[[http://criticmarkup.com/][CriticMarkup]] is a Markdown-like syntax for authors and editors to track changes to documents in plain text. 

{++Additions++} and {--Deletions--} and {~~substitutions~>replacements~~} appear inline just like Markdown syntax.

#+name: writing-criticmarkup
#+BEGIN_SRC emacs-lisp
(use-package cm-mode)
...
Enter CriticMarkup-mode by calling =M-x cm-mode= -- this will highlight items in CM syntax for easy visibility.

To add CriticMarkup, use these keybindings:
...
To toggle a "track changes" mode where your edits appear in the buffer as marked-up {++changes++}, hit =C-c * F=. 

Once you have a buffer with CM markup in it, you can walk through the changes and accept/reject/skip suggestions interactively with =C-c * I=.
...
#+END_SRC
*** Don't nag me to specify upstream branch when pushing

#+name: coding-magit-push-verify
#+BEGIN_SRC emacs-lisp
...
I like to store my python-env directory outside ~/.emacs.d. 

Partly because you end up symlinking deep into /Library/Frameworks/Python.framework, and when I've got that action happening in my ~/.emacs.d/, Dropbox runs off and cries.

#+name: coding-python-env
#+BEGIN_SRC emacs-lisp
...
I've been trying my hand at writing interactive fiction using the Inform7 language. 

Inform comes with an amazing IDE of its own, and this Emacs mode will probably never match its power, unfortunately... but I've still been playing with it.

#+name: coding-inform7-mode
#+BEGIN_SRC emacs-lisp
