...
:PROPERTIES:
:CUSTOM_ID: how_do_i_use
:END:

Hey, I'm not gonna tell you what to do. But here are a couple of suggestions:

1. Sit back, pour yourself a beer, and read through this documentation.
   - I've tried to make it reasonably readable, even for folks who are relatively new to futzing around with Emacs configs.
2. Pour yourself another beer.
3. Trawl through the code snippets contained in this documentation
   - Try them in your own Emacs setup. See what works for you.
4. Repeat steps 2 and 3. Steal anything that's useful.
5. Clone the repo and check out the emacs-init.org file itself.
6. If you're feeling really adventurous, try installing the whole shebang [[Installation][as per the instructions below]].
...
If you really want to take the plunge and try this thing out, follow these simple steps...
...
Before we get too deep into my settings, I should explain a little about the magic that makes my setup work.
...
Don't worry, I'll explain how it works in a second. 
...
When this document is tangled and loaded by org-babel, all the elisp code snippets contained within it are tangled and loaded. 
...
Any edits or changes, even if they're experimental, are made in the org-mode files instead of directly in the .el files. This way, I'm ensuring my documentation always stays up to date.
...
The default package manager in Emacs 24 is super handy. Use =package-list-packages= to browse packages on all source repos, then install/uninstall/update them from within Emacs. Super slick.
...
Git submodules are an option. But they also make me a little ill.

What I really want is for my .emacs setup to contain only *my* configuration, and for any public packages to be downloaded from the interwebs on-demand. And I want a really nice, clean, easy-to-understand method for doing all this.
...
Normally in an Emacs config, you'll see a lot of =(require 'packagename)= expressions, much like I've done with Cask and Pallet packages above. These =require= statements will find the package in the list of =load-path= directories and actually load/initialize all of the package's elisp.
...
It's sometimes difficult to know where or how to use =autoload=, however -- for example, there are times when you may want to configure some parts of a package at Emacs startup time, but wait until the package is fully loaded to fire off other config options.
