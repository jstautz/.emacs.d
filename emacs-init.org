...
[fn:2] One downside to this is that on every cask init, you'll pull down the latest versions of each package. *This may include unstable bleeding-edge versions.*
...
This is a problem with *any* package management setup in Emacs. It's extraordinarily difficult to create a setup that says "I want to use only this specific version of package x and this specific version of package y." 
...
In any case, this is a known issue with my current setup that I'm still working on.
...
Note that the reverse is also true. If I remove a package using the built-in package manager, Pallet removes its line from my Cask file. So handy.
...
This is generally a good idea.[fn:3] There's a lot of good stuff in the CL package that many other packages rely on.
...
[fn:3] I should probably revisit this at some point. Apparently as of Emacs 24, this should be =(use-package cl-lib)=? Will need to investigate and test. Later.
...
Some of these are obviously going to be different in your setup, so you'll want to change these accordingly.
...
Another obnoxious default to change: the requirement that you actually type the letters "y" "e" and "s" at a yes/no prompt. Whose idea was that? 
...
#+end_src
**** TODO note that I've currently disabled "native" fullscreen because the way it handles other apps on multi-monitor setups annoys me.
...
In the first option, =auto-fill-mode= will automatically insert linebreaks at a certain column number, preventing any lines from being longer than your screen's viewable area. setting fill-column defines the column number at which Emacs will auto-insert a linebreak.
...
I've since discovered wrap-to-fill-column-mode (was packaged with nxhtml-mode -- I've pulled it out in my .emacs.d directory). This allows me to use visual-line-mode for word wrapping, yet keeps the lines to a reasonable length. It also centers the text nicely in my window.
