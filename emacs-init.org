...
The default package manager in Emacs 24 is super handy. Use =package-list-packages= to browse packages on all source repos, then install/uninstall/update them. Slick.

Before using it, I want to add a few other sources (notably marmalade and melpa) to my package source list.
...
The =Cask= file in my .emacs.d/ lists the packages that my setup depends on, as well as specifying some source repositories (the same ones I specify for package.el, above).
...
So =package.el= lets me browse and install packages from within Emacs... but Cask manages packages outside emacs based on a Cask file... so how am I supposed to keep these in sync?

Enter Pallet.

If I install a new package via =M-x list-packages=, Pallet automatically updates my Cask file and keeps everything in lockstep. This way I don't lose track of anything installed via package.el -- it's all still in my config and can be pulled down via =cask install=. 

Note that the reverse is true if I remove a package using the built-in package manager -- Pallet removes its line from my Cask file.
...
*** bzg's big fringe mode (experimental)
...
One nice feature of Sublime Text is the [[http://i.stack.imgur.com/SkFIc.png][Minimap]]: a small scaled-down overview of the code in your buffer, so you can get a sense of your current location in relation to the rest of the file.

Sublimity is a package that provides this functionality for emacs.

Actually, I don't like this. It fucks with my scrolling. 

#+name: coding-minimap-sublimity
#+begin_src emacs-lisp
